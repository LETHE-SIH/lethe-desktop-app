...
export function StatsCards() {
  ...
  const fetchDashboard = async () => {
    try {
      const res = await fetch("https://lethe-api.zerodev.me/api/v1/public/dashboard");
      ...
      if (data?.physical_disks) {
        ...
        if (updatedDashboard.wipe_in_progress === 0) {
          try {
            const wipeRes = await fetch("https://lethe-api.zerodev.me/api/v1/public/wipe/status");
